...
from .device_driver import DeviceDriver, DeviceInfo, DriverType, SwipeResult, TapResult, TextResult
...
class LaixiDriver(DeviceDriver):
        ...
        for attempt in range(self.max_reconnect_attempts):
            try:
                ...
                self._websocket = await asyncio.wait_for(
                    websockets.connect(self.websocket_url), timeout=self.timeout
                )
    ...
    async def _send_command(self, command: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        async with self._lock:
            ...
            try:
                ...
                response_text = await asyncio.wait_for(self._websocket.recv(), timeout=self.timeout)
    ...
    async def tap(self, device_id: str, x: float, y: float, duration_ms: int = 100) -> TapResult:
        """
        화면 탭 (백분율 좌표)
        ...
        try:
            ...
            return TapResult(
                success=response is not None, actual_x=x, actual_y=y, duration_ms=duration_ms
            )
        except Exception as e:
            return TapResult(
                success=False, actual_x=x, actual_y=y, duration_ms=duration_ms, error=str(e)
            )

    async def swipe(
        self, device_id: str, x1: float, y1: float, x2: float, y2: float, duration_ms: int = 300
    ) -> SwipeResult:
        """
        화면 스와이프 (백분율 좌표)
        ...
        try:
            ...
            return SwipeResult(success=response is not None, path=path, duration_ms=duration_ms)
        except Exception as e:
            return SwipeResult(success=False, path=[], duration_ms=duration_ms, error=str(e))

    async def text(self, device_id: str, content: str, use_clipboard: bool = True) -> TextResult:
        """
        ...
        try:
            ...
            return TextResult(success=True, typed_text=content)
        except Exception as e:
            return TextResult(success=False, typed_text="", error=str(e))
    ...
    async def screenshot(self, device_id: str, save_path: Optional[str] = None) -> Optional[bytes]:
        """스크린샷 캡처"""
        if save_path:
            await self._send_command(
                {"action": "screen", "comm": {"deviceIds": device_id, "savePath": save_path}}
            )
    ...
    async def show_toast(self, device_id: str, message: str) -> bool:
        """Toast 메시지 표시"""
        response = await self._send_command(
            {"action": "Toast", "comm": {"deviceIds": device_id, "content": message}}
        )
        return response is not None
    ...
    async def get_current_app(self, device_id: str) -> Optional[Dict]:
        ...
        return await self._send_command(
            {"action": "CurrentAppInfo", "comm": {"deviceIds": device_id}}
        )

    async def quick_swipe(self, device_id: str, direction: str) -> bool:
        """
        빠른 스와이프 (Laixi 네이티브)

        direction: "up", "down", "left", "right"
        """
        mask_map = {"up": "6", "down": "7", "left": "8", "right": "9"}
