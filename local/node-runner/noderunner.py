...
class LaixiClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._ws = await asyncio.wait_for(
                websockets.connect(self.url, ping_interval=20, ping_timeout=10), timeout=5.0
            )
    ...
    async def _restart_laixi(self):
        ...
        try:
            ...
            if sys.platform == "win32":
                subprocess.run(
                    ["taskkill", "/f", "/im", "Laixi.exe"], capture_output=True, timeout=10
                )
            else:
            ...
            if os.path.exists(LAIXI_PATH):
                subprocess.Popen([LAIXI_PATH], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    async def execute(self, action: str, device_id: str, params: dict) -> dict:
        ...
        if action == "list":
            ...
            return {"success": resp is not None, "data": {"watched_sec": duration}}
            ...
            resp = await self._send(
                {"action": "adb", "comm": {"deviceIds": target, "cmd": cmd_str}}
            )
        ...
        # ==================== P1: 상호작용 기능 ====================
            ...
            resp = await self._send(
                {"action": "BasisOperate", "comm": {"deviceIds": target, "type": "4"}}
            )
            return {"success": resp is not None}
            ...
            resp = await self._send(
                {"action": "BasisOperate", "comm": {"deviceIds": target, "type": "3"}}
            )
            return {"success": resp is not None}
            ...
            resp = await self._send(
                {"action": "BasisOperate", "comm": {"deviceIds": target, "type": "15"}}
            )
            return {"success": resp is not None}
            ...
            resp = await self._send(
                {"action": "BasisOperate", "comm": {"deviceIds": target, "type": "14"}}
            )
            return {"success": resp is not None}

        elif action == "current_app":
            # 현재 앱 정보 (CurrentAppInfo)
            resp = await self._send({"action": "CurrentAppInfo", "comm": {"deviceIds": target}})
            if resp and resp.get("StatusCode") == 200:
                result = resp.get("result", "{}")
                if isinstance(result, str):
                    try:
        ...
        # ==================== P2: 고급 기능 ====================
            ...
            resp = await self._send(
                {"action": "screen", "comm": {"deviceIds": target, "savePath": save_path}}
            )
            ...
            resp = await self._send(
                {"action": "writeclipboard", "comm": {"deviceIds": target, "content": text}}
            )
            return {"success": resp is not None}

        elif action == "clipboard_get":
            # 클립보드 읽기 (getclipboard) - 단일 디바이스만
            resp = await self._send({"action": "getclipboard", "comm": {"deviceIds": target}})
            if resp:
            ...
            resp = await self._send(
                {"action": "Toast", "comm": {"deviceIds": target, "content": message}}
            )
            return {"success": resp is not None}
            ...
            resp = await self._send(
                {"action": "BasisOperate", "comm": {"deviceIds": target, "type": "1"}}
            )
            return {"success": resp is not None}
            ...
            resp = await self._send(
                {"action": "BasisOperate", "comm": {"deviceIds": target, "type": "2"}}
            )
            return {"success": resp is not None}
            ...
            if not package:
                return {"success": False, "error": "Package name required"}
            resp = await self._send(
                {"action": "adb", "comm": {"deviceIds": target, "cmd": f"am force-stop {package}"}}
            )
            return {"success": resp is not None}
            ...
            resp = await self._send(
                {"action": "adb", "comm": {"deviceIds": target, "cmd": f"input keyevent {keycode}"}}
            )
            return {"success": resp is not None}
...
class NodeRunner:
    ...
    async def _connect_and_run(self):
        ...
        # Central 연결
        self._ws = await asyncio.wait_for(
            websockets.connect(CENTRAL_URL, ping_interval=20, ping_timeout=10), timeout=30.0
        )

        # HELLO 전송
        await self._ws.send(
            json.dumps(
                {"type": "HELLO", "node_id": NODE_ID, "device_count": self._laixi.device_count}
            )
        )
    ...
    async def _send_result(
        ...
        """결과 전송"""
        result = {"type": "RESULT", "command_id": command_id, "success": success}
        if data:
            result["data"] = data
        if error:
            result["error"] = error
