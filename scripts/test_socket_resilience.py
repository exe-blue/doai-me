...
ESTABLISH_SCRIPT_PATH = (
    Path(__file__).parent.parent / "gateway" / "scripts" / "laixi" / "establish_connection.js"
)
...
async def test_laixi_crash_recovery() -> TestResult:
    ...
    try:
        ...
        ws = await asyncio.wait_for(
            websockets.connect(LAIXI_WS_URL, ping_interval=10, ping_timeout=5), timeout=10.0
        )
        ...
        if sys.platform == "win32":
            ...
            kill_result = subprocess.run(
                ["taskkill", "/f", "/im", "touping.exe"], capture_output=True, text=True, timeout=10
            )
        ...
        while time.time() - start_time < config["max_reconnect_wait"]:
            ...
            try:
                ws = await asyncio.wait_for(
                    websockets.connect(LAIXI_WS_URL, ping_interval=10, ping_timeout=5), timeout=5.0
                )
    ...
    try:
        ...
        for i in range(len(commands)):
            try:
                ...
                logger.info(
                    f"  [RECV] #{i+1} StatusCode={resp_data.get('StatusCode')} ({recv_time:.3f}s)"
                )
    ...
    try:
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
            writer = csv.DictWriter(
                csvfile, fieldnames=["timestamp", "elapsed_sec", "rss_mb", "vms_mb", "percent"]
            )
            writer.writeheader()
            ...
            while time.time() - start_time < duration_sec:
                ...
                try:
                    ...
                    if len(snapshots) % (600 // interval_sec) == 0:
                        increase = (
                            ((rss_mb - initial_memory) / initial_memory * 100)
                            if initial_memory > 0
                            else 0
                        )
                        logger.info(
...
예시:
  ...
  python test_socket_resilience.py --test memory --duration 1
    ...
    parser.add_argument(
        "--duration", "-d", type=float, default=24, help="메모리 테스트 시간 (시간 단위, 기본: 24)"
    )
