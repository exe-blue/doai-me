...
"""
        ...
        execute_command(
            ssh, f"cat > /opt/doai-me/n8n/docker-compose.yml << 'EOF'\n{n8n_compose}\nEOF"
        )
...
"""
        ...
        execute_command(
            ssh, f"cat > /opt/doai-me/backend/Dockerfile << 'EOF'\n{fastapi_dockerfile}\nEOF"
        )
        execute_command(
            ssh, f"cat > /opt/doai-me/backend/docker-compose.yml << 'EOF'\n{fastapi_compose}\nEOF"
        )
...
"""
        ...
        execute_command(
            ssh, f"cat > /opt/doai-me/gateway/server.js << 'EOF'\n{gateway_server}\nEOF"
        )
        execute_command(
            ssh, f"cat > /opt/doai-me/gateway/package.json << 'EOF'\n{gateway_package}\nEOF"
        )
        execute_command(
            ssh, f"cat > /opt/doai-me/gateway/Dockerfile << 'EOF'\n{gateway_dockerfile}\nEOF"
        )
        execute_command(
            ssh, f"cat > /opt/doai-me/gateway/docker-compose.yml << 'EOF'\n{gateway_compose}\nEOF"
        )
        ...
        print("\n[CHECK] Port Status:")
        execute_command(
            ssh, "ss -tlnp | grep -E ':(8000|3100|5678)' || echo 'Ports not yet listening'"
        )
