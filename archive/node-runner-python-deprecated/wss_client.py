...
def generate_signature(payload: dict, secret_key: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"Invalid Base64 format for secret_key: {e}. Please check NODE_SECRET_KEY environment variable.")
...
def build_heartbeat(config: NodeConfig, status: str, device_snapshot: list, 
                    active_tasks: int = 0, resources: dict = None) -> dict:
    """HEARTBEAT 메시지 빌드"""
    payload = {
        "status": status,
...
def build_result(config: NodeConfig, command_id: str, status: str, 
                 device_results: list, error: str = None) -> dict:
    """RESULT 메시지 빌드"""
...
async def main():
    ...
    client = NodeRunnerClient(
        ...
        device_snapshot_provider=get_device_snapshot
    )
