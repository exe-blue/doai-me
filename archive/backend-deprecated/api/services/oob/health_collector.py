...
class NodeMetrics:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.adb_server_ok
            and self.unauthorized_count < 3
            and self.device_loss_pct < 0.1  # 10% 미만 손실
        )
...
class HealthCollector:
    ...
    def _register_node_unsafe(
        self, node_id: str, expected_devices: int = 0, tailscale_ip: Optional[str] = None
    ) -> NodeHealth:
        """노드 등록 (락 없이 - 호출자가 락을 보유해야 함)"""
        if node_id not in self._nodes:
            self._nodes[node_id] = NodeHealth(node_id=node_id, tailscale_ip=tailscale_ip)
    ...
    async def update_node_metrics(self, node_id: str, metrics_data: dict) -> NodeHealth:
        """
        노드 메트릭 업데이트
        ...
        async with self._lock:
            ...
            metrics = NodeMetrics(
                ...
                or metrics_data.get("device_count", 0),
            ...
            logger.debug(
                f"Metrics updated for {node_id}: devices={metrics.device_count_adb}, adb_ok={metrics.adb_server_ok}"
            )
    ...
    def get_unhealthy_nodes(self) -> List[NodeHealth]:
        """비정상 노드 목록"""
        return [node for node in self._nodes.values() if node.status != ConnectionStatus.CONNECTED]

    async def record_recovery(self, node_id: str, recovery_type: str):  # 'soft', 'restart', 'box'
        """복구 실행 기록"""
        async with self._lock:
            if node_id in self._nodes:
                node = self._nodes[node_id]
    ...
    async def can_execute_recovery(
        self, node_id: str, recovery_type: str, cooldown_minutes: int
    ) -> bool:
        """복구 실행 가능 여부 (쿨다운 체크)"""
        async with self._lock:
    ...
    async def sync_from_database(self):
        ...
        try:
            # nodes 테이블에서 정보 조회
            result = (
                self._supabase.table("nodes")
                .select("node_id, name, capacity, tailscale_ip, status_v2")
                .execute()
            )
