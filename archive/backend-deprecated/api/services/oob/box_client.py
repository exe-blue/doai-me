...
class BoxClient:
    ...
    async def _send_command(self, hex_command: str, expect_response: bool = False) -> BoxResponse:
        """
        박스에 TCP 명령 전송
                ...
                if expect_response:
                    try:
                        raw_response = await asyncio.wait_for(
                            reader.read(1024), timeout=1.0  # 응답 타임아웃 짧게
                        )
                        logger.debug(
                            f"Box response: {raw_response.hex() if raw_response else 'empty'}"
                        )
    ...
    async def discover_protocol(
        cls, host: str, port: int = 56666, test_command: str = "AA 01 88 84 01 00 DD"
    ) -> dict:
        """
