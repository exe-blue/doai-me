...
class WatchRequest(BaseModel):
    ...
    target_device_ids: Optional[List[str]] = Field(
        None, description="대상 디바이스 ID 목록 (None이면 전체)"
    )
    watch_duration_seconds: int = Field(30, ge=5, le=3600, description="시청 시간 (초)")
    mode: WatchMode = Field(WatchMode.DIRECT_URL, description="시청 모드")
...
async def dispatch_youtube_watch(
    client: LaixiClient, device_ids: str, video_url: str, video_id: str, title: Optional[str] = None
) -> bool:
    """
    YouTube 영상 시청 명령 전송
...
async def list_laixi_devices():
    ...
    """
    if not HAS_LAIXI:
        raise HTTPException(status_code=503, detail="Laixi 클라이언트가 설치되지 않음")
    ...
    try:
        ...
        if not await client.connect():
            raise HTTPException(
                status_code=503, detail="Laixi 서버에 연결할 수 없음 (touping.exe 실행 확인)"
            )
        ...
        return DevicesResponse(success=True, devices=devices, count=len(devices))
...
async def watch_youtube_video(request: WatchRequest):
    ...
    try:
        ...
        if not await client.connect():
            raise HTTPException(status_code=503, detail="Laixi 서버에 연결할 수 없음")
...
async def stop_youtube(request: StopRequest):
    ...
    try:
        ...
        return {"success": True, "message": "YouTube 종료 및 홈 화면 이동 완료"}
...
async def take_screenshot(device_id: str, save_path: str = "d:\\screenshots"):
    """
    디바이스 스크린샷
