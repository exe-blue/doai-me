...
class YouTubeChannelMonitor:
    ...
    async def get_active_channels(self) -> List[Dict[str, Any]]:
        ...
        try:
            result = (
                self.client.table("youtube_channels").select("*").eq("is_active", True).execute()
            )
    ...
    def parse_rss(self, xml_content: str, channel_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            channel_id = (
                ...
                if channel_id_match
                else channel_info.get("channel_id", "unknown")
            )
    ...
    async def get_pending_videos(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            result = (
                ...
                .select("*")
                .eq("status", "PENDING")
                .order("priority", desc=True)  # URGENT > HIGH > NORMAL > LOW
                .order("created_at", desc=False)  # 오래된 것 먼저
                .limit(limit)
                .execute()
            )
    ...
    async def scan_all_channels(self) -> Dict[str, Any]:
        ...
        result = {"scanned_channels": 0, "new_videos": 0, "errors": 0, "channels": []}
