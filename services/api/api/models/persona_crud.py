...
class PersonaCreateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=500, description="페르소나 설명 (성격, 관심사 등)"
    )
    age: Optional[int] = Field(None, ge=13, le=100, description="나이")
    gender: Optional[str] = Field(None, description="성별 (male/female/other)")
    interests: List[str] = Field(default_factory=list, max_length=20, description="관심사 목록")
    traits: Optional[PersonaTraitsInput] = Field(
        None, description="성격 특성 (미입력시 기본값 사용)"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "traits": {"curiosity": 90, "enthusiasm": 75, "humor": 60},
            }
        }
...
class PersonaUpdateRequest(BaseModel):
    ...
    existence_state: Optional[str] = Field(
        None, description="존재 상태 (active/waiting/fading/void)"
    )
...
class ExecuteSearchRequest(BaseModel):
    ...
    keyword: Optional[str] = Field(None, description="검색할 키워드 (미입력시 자동 생성)")
    watch_video: bool = Field(default=True, description="검색 후 첫 번째 영상 시청 여부")
    watch_duration_seconds: Optional[int] = Field(
        None, ge=10, le=600, description="시청 시간 (초, 미입력시 랜덤)"
    )
    like_probability: float = Field(default=0.1, ge=0, le=1, description="좋아요 확률")
...
class PersonalityDriftResponse(BaseModel):
    ...
    drift_score: float = Field(
        ..., ge=0, le=1, description="성격 변화 정도 (0=변화없음, 1=완전변화)"
    )
    drift_direction: str = Field(..., description="변화 방향 (expanding/narrowing/shifting)")

    # 카테고리 분석
    top_categories: List[CategorySearchStats] = Field(
        default_factory=list, description="상위 검색 카테고리"
    )

    # 관심사 비교
    original_interests: List[str] = Field(default_factory=list)
    suggested_interests: List[str] = Field(
        default_factory=list, description="검색 기반 추천 관심사"
    )
                ...
                "top_categories": [
                    {
                        ...
                        "search_count": 15,
                        "percentage": 45.5,
                        "sample_keywords": ["AI 뉴스", "GPT-5"],
                    },
                    {
                        "category": "게임",
                        "search_count": 8,
                        "percentage": 24.2,
                        "sample_keywords": ["롤", "발로란트"],
                    },
                ],
...
class UpdateInterestsRequest(BaseModel):
    ...
    min_search_count: int = Field(default=3, ge=1, description="관심사로 추가할 최소 검색 횟수")
    auto_remove_unused: bool = Field(
        default=False, description="검색하지 않는 관심사 자동 제거 여부"
    )
    confirm: bool = Field(
        default=False, description="True면 실제 업데이트 수행, False면 미리보기만"
    )
