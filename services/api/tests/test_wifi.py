...
from services.wifi_service import DeviceWifiStatus, S9Coordinates, WifiService
...
class TestWifiService:
    ...
    async def test_verify_connection_success(self, wifi_service):
        ...
        async def mock_check(device_id):
            return DeviceWifiStatus(
                device_id=device_id, connected=True, ssid="JH-Wifi", ip_address="192.168.1.100"
            )

        wifi_service.check_wifi_status = mock_check

        report = await wifi_service.verify_connection(
            target_ssid="JH-Wifi", device_ids=["d1", "d2", "d3"]
        )
    ...
    async def test_verify_connection_partial(self, wifi_service):
        ...
        async def mock_check(device_id):
            call_count[0] += 1
            if call_count[0] <= 2:
                return DeviceWifiStatus(device_id=device_id, connected=True, ssid="JH-Wifi")
            else:
                return DeviceWifiStatus(device_id=device_id, connected=False, ssid=None)
    ...
    async def test_wifi_connect_flow(self):
        ...
        try:
            ...
            result = await wifi.connect_wifi(
                ssid=test_ssid, password=test_password, device_ids="all"
            )
...
async def manual_test():
    ...
    try:
        ...
        result = await wifi.connect_wifi(ssid="JH-Wifi", password="jh000Aa@@")
