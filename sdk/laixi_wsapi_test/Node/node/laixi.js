 ...
 * @returns 
 */
module.exports.toastMsg = (msg = "这是来喜弹出的一个Toast提示", deviceIds = "all") => {
 ...
 * @returns 
 */
module.exports.pointEvent = (deviceIds, mask, x = "0.5", y = "0.5", endx = "0", endy = "0", delta = "2") => {
    return JSON.stringify({
        "action": "PointerEvent",
        "comm": {
 ...
 * @returns 
 */
module.exports.listKeywordPackages = (keyword, deviceIds = "all") => {
    // 키워드 유효성 검사 (커맨드 인젝션 방지)
    if (!keyword || !/^[A-Za-z0-9_-]+$/.test(keyword)) {
        throw new Error('Invalid keyword: only letters, numbers, underscores and hyphens allowed');
    }
    return JSON.stringify({
        "action": "ADB",
        "comm": {
