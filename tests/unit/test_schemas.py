...
class TestVideoQueueSchemas:
    ...
    def test_video_queue_create_defaults(self):
        """기본값 테스트"""
        request = VideoQueueCreate(youtube_video_id="test123", title="기본값 테스트")
    ...
    def test_video_queue_update_partial(self):
        """대기열 부분 업데이트 요청"""
        update = VideoQueueUpdate(priority=8, status=QueueStatus.READY)
...
class TestCommentPoolSchemas:
    ...
    def test_comment_pool_create_korean(self):
        ...
        comment = CommentPoolCreate(
            content="좋은 영상이네요!", category=CommentCategory.POSITIVE, language="ko", weight=100
        )
    ...
    def test_comment_pool_create_emoji(self):
        """이모지 댓글"""
        comment = CommentPoolCreate(content="👍", category=CommentCategory.EMOJI, language="mixed")
...
class TestDispatchRequestSchemas:
    ...
    def test_dispatch_request_basic(self):
        """기본 분배 요청"""
        request = DispatchRequest(device_percent=0.5, workstation_id="WS01")
...
class TestQueueSummarySchemas:
    ...
    def test_queue_summary_with_values(self):
        """값이 있는 요약 통계"""
        summary = QueueSummary(
            total_items=100, pending=20, ready=30, executing=10, completed=35, failed=5, cancelled=0
        )
...
class TestWorkloadSchemas:
    ...
    def test_batch_config_custom(self):
        """BatchConfig 커스텀 값"""
        config = BatchConfig(
            batch_size_percent=30, batch_interval_seconds=120, cycle_interval_seconds=400
        )
