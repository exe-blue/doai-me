...
class HumanSimulatorPython:
    ...
    def __init__(self, config: dict = None):
        config = config or {}
        self.idle_config = {
            'delay_min_ms': 3000, 'delay_max_ms': 7000, 'click_error_px': 20,
            'watch_min_seconds': 5, 'watch_max_seconds': 60,
            'like_probability': 0.10, 'comment_probability': 0.05,
            **(config.get('idle', {}))
        }
        self.queue_config = {
            'delay_min_ms': 5000, 'delay_max_ms': 10000, 'click_error_px': 20,
            'ad_skip_wait_min_ms': 7000, 'ad_skip_wait_max_ms': 20000,
            'watch_min_seconds': 120, 'watch_max_seconds': 600,
            'like_probability': 0.10, 'comment_probability': 0.05,
            'random_actions': {
    ...
    def generate_ad_skip_delay(self) -> int:
        return self.random_int(self.queue_config['ad_skip_wait_min_ms'],
                               self.queue_config['ad_skip_wait_max_ms'])

    def generate_watch_duration(self, mode: str = 'idle', video_duration_seconds: int = None) -> int:
        config = self.queue_config if mode == 'queue' else self.idle_config
    ...
    def generate_click_position(self, target_x: int, target_y: int, mode: str = 'idle') -> Tuple[int, int]:
        click_error = (self.queue_config if mode == 'queue' else self.idle_config)['click_error_px']
        x = max(0, min(self.screen_width - 1, target_x + self.random_int(-click_error, click_error)))
        y = max(0, min(self.screen_height - 1, target_y + self.random_int(-click_error, click_error)))
        return (round(x), round(y))

    def should_like(self, mode: str = 'idle') -> bool:
        return self.should_occur((self.queue_config if mode == 'queue' else self.idle_config)['like_probability'])

    def should_comment(self, mode: str = 'idle') -> bool:
        return self.should_occur((self.queue_config if mode == 'queue' else self.idle_config)['comment_probability'])
