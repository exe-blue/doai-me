...
from shared.workload_engine import WorkloadState
...
class TestWorkloadStatusTransitions:
    ...
    def test_pending_to_listing(self):
        """PENDING -> LISTING"""
        state = WorkloadState(workload_id="wl-001", status=WorkloadStatus.PENDING)
    ...
    def test_listing_to_executing(self):
        """LISTING -> EXECUTING"""
        state = WorkloadState(workload_id="wl-001", status=WorkloadStatus.LISTING)
    ...
    def test_executing_to_recording(self):
        """EXECUTING -> RECORDING"""
        state = WorkloadState(workload_id="wl-001", status=WorkloadStatus.EXECUTING)
    ...
    def test_recording_to_waiting(self):
        """RECORDING -> WAITING"""
        state = WorkloadState(workload_id="wl-001", status=WorkloadStatus.RECORDING)
    ...
    def test_waiting_to_listing(self):
        ...
        state = WorkloadState(workload_id="wl-001", status=WorkloadStatus.WAITING)
    ...
    def test_paused_to_previous(self):
        """PAUSED -> 이전 상태 (재개)"""
        state = WorkloadState(workload_id="wl-001", status=WorkloadStatus.PAUSED)
...
class TestWorkloadEngineCalculateNextVideo:
    ...
    def test_first_video(self):
        ...
        state = WorkloadState(workload_id="wl-001", current_video_index=0)
    ...
    def test_last_video(self):
        ...
        state = WorkloadState(workload_id="wl-001", current_video_index=2)
...
class TestWorkloadEngineShouldContinue:
    ...
    def test_continue_with_remaining_videos(self):
        """남은 영상이 있으면 계속"""
        state = WorkloadState(workload_id="wl-001", current_video_index=0, should_stop=False)
    ...
    def test_stop_when_all_done(self):
        """모든 영상 완료 시 종료"""
        state = WorkloadState(workload_id="wl-001", current_video_index=5, should_stop=False)
    ...
    def test_stop_when_flag_set(self):
        """정지 플래그 설정 시 종료"""
        state = WorkloadState(workload_id="wl-001", current_video_index=2, should_stop=True)
    ...
    def test_stop_when_cancelled(self):
        """취소 상태면 종료"""
        state = WorkloadState(
            workload_id="wl-001", status=WorkloadStatus.CANCELLED, current_video_index=2
        )
...
class TestWorkloadConfig:
    ...
    def test_default_config(self):
        """기본 설정"""
        config = WorkloadCreate(name="테스트 워크로드", video_ids=["v1", "v2", "v3"])
    ...
    def test_with_batch_config(self):
        """배치 설정 포함"""
        batch_config = BatchConfig(batch_size_percent=30, batch_interval_seconds=120)

        config = WorkloadCreate(name="배치 워크로드", video_ids=["v1"], batch_config=batch_config)

        assert config.batch_config.batch_size_percent == 30

    def test_with_watch_config(self):
        """시청 설정 포함"""
        watch_config = WatchConfig(watch_duration_min=60, like_probability=0.30)

        config = WorkloadCreate(name="시청 워크로드", video_ids=["v1"], watch_config=watch_config)
