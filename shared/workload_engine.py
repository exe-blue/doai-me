...
class WorkloadEngine:
    ...
    def __init__(
        self, registry: Optional[DeviceRegistry] = None, executor: Optional[BatchExecutor] = None
    ):
        """
        WorkloadEngine 초기화
        ...
        self.on_cycle_complete: Optional[Callable[[str, WorkloadCycleResult], Awaitable[None]]] = (
            None
        )
    ...
    async def create_workload(self, request: WorkloadCreate) -> WorkloadResponse:
        ...
        await self._log(
            workload_id, LogLevel.INFO, f"워크로드 생성: {len(request.video_ids)}개 영상"
        )
    ...
    async def get_workload(self, workload_id: str) -> Optional[WorkloadResponse]:
        ...
        try:
            result = (
                self.client.table("workloads").select("*").eq("id", workload_id).single().execute()
            )
    ...
    async def get_workloads(
        self, status: Optional[WorkloadStatus] = None, limit: int = 50
    ...
    async def update_workload_status(
        self, workload_id: str, status: WorkloadStatus, **kwargs: Any
    ) -> bool:
        """워크로드 상태 업데이트"""
        try:
            ...
            result = (
                self.client.table("workloads").update(update_data).eq("id", workload_id).execute()
            )
    ...
    async def cancel_workload(self, workload_id: str) -> bool:
        ...
        return await self.update_workload_status(workload_id, WorkloadStatus.CANCELLED)
    ...
    async def _run_workload(
        self, workload_id: str, workload: WorkloadResponse, state: WorkloadState
    ) -> None:
        """
        워크로드 실행 메인 루프
        ...
        try:
            ...
            for idx in range(state.current_video_index, len(video_ids)):
                ...
                if not video:
                    await self._log(
                        workload_id, LogLevel.WARN, f"영상 정보 없음: {video_id}", video_id=video_id
                    )
                    continue
                ...
                # 상태 업데이트: EXECUTING
                await self.update_workload_status(
                    workload_id, WorkloadStatus.EXECUTING, current_video_index=idx
                )

                # 배치 실행
                cycle_result = await self._execute_video_cycle(
                    workload_id, video, batch_config, state, workload.target_workstations
                )
                ...
                if idx < len(video_ids) - 1 and not state.should_stop:
                    ...
                    await self.update_workload_status(
                        workload_id, WorkloadStatus.WAITING, next_cycle_at=wait_until.isoformat()
                    )
            ...
            # 완료 처리
            final_status = (
                WorkloadStatus.CANCELLED if state.should_stop else WorkloadStatus.COMPLETED
            )
            ...
            await self.update_workload_status(workload_id, WorkloadStatus.ERROR)

            await self._log(workload_id, LogLevel.ERROR, f"워크로드 오류: {e}")
    ...
    async def _execute_video_cycle(
        ...
        cycle_result = WorkloadCycleResult(
            video_id=video.video_id, video_title=video.title, started_at=started_at
        )
    ...
    def _make_device_complete_callback(
        self, workload_id: str, state: WorkloadState
    ...
    async def _get_video_info(self, video_id: str) -> Optional[VideoTarget]:
        """영상 정보 조회"""
        try:
            result = self.client.table("videos").select("*").eq("id", video_id).single().execute()
    ...
    async def _record_cycle_result(
        self, workload_id: str, video_id: str, cycle_result: WorkloadCycleResult
    ) -> None:
        """사이클 결과를 DB에 기록"""
        try:
            # 영상 완료 카운트 업데이트
                    ...
                    "completed_count": (
                        self.client.rpc(
                            "increment",
                            {
                                ...
                                "amount": cycle_result.total_success,
                            },
                        )
                        if False
                        else cycle_result.total_success
                    )  # RPC 대신 직접 업데이트
                }
            ...
            for batch in cycle_result.batch_results:
                for device_result in batch.device_results:
                    history_data = {
                        ...
                        "completed_at": (
                            device_result.completed_at.isoformat()
                            if device_result.completed_at
                            else None
                        ),
    ...
    async def get_workload_logs(
        self, workload_id: str, level: Optional[LogLevel] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """워크로드 로그 조회"""
        try:
            query = self.client.table("workload_logs").select("*").eq("workload_id", workload_id)
    ...
    def _to_response(self, data: Dict[str, Any]) -> WorkloadResponse:
        ...
        return WorkloadResponse(
            ...
            next_cycle_at=(
                datetime.fromisoformat(data["next_cycle_at"]) if data.get("next_cycle_at") else None
            ),
            created_at=(
