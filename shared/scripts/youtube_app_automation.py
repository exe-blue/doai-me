...
class YouTubeAppAutomation:
    ...
    def __init__(
        self, laixi: Optional[LaixiClient] = None, coords: Optional[YouTubeCoordinates] = None
    ):
        """
    ...
    async def execute(self, task: WatchTask) -> WatchResult:
        ...
        try:
            ...
            actual_watched = await self._watch_video(
                device_id, watch_duration, task.duration_seconds or 180
            )
            ...
            if task.should_comment and task.device_logged_in and task.comment_text:
                ...
                result.commented = await self._write_comment(device_id, task.comment_text)
    ...
    async def _launch_youtube(self, device_id: str) -> bool:
        ...
        try:
            ...
            # YouTube 앱 실행
            command = f"am start -n {self.youtube_package}/{self.youtube_activity}"
            return await self.laixi.execute_adb(device_id, command)
        except Exception as e:
            logger.error(f"YouTube 실행 실패: {e}")
            return False

    async def _search_video(self, device_id: str, keyword: str) -> bool:
        """영상 검색"""
        try:
            # 검색 아이콘 클릭
            await self.laixi.tap(device_id, self.coords.search_icon[0], self.coords.search_icon[1])
            await asyncio.sleep(random.uniform(1.0, 2.0))
            ...
            await self.laixi.tap(
                device_id, self.coords.search_input[0], self.coords.search_input[1]
            )
            await asyncio.sleep(0.5)

            # 붙여넣기 (Ctrl+V 또는 롱프레스 후 붙여넣기)
            # ADB input 사용
            await self.laixi.execute_adb(device_id, "input keyevent 279")  # KEYCODE_PASTE
            await asyncio.sleep(random.uniform(0.5, 1.0))

            # 검색 실행 (Enter)
            await self.laixi.execute_adb(device_id, "input keyevent 66")  # KEYCODE_ENTER
    ...
    async def _find_and_click_video(self, device_id: str, title: str, max_scrolls: int = 5) -> int:
        """
        영상 찾기 및 클릭
        ...
        try:
            ...
            for scroll in range(max_scrolls):
                ...
                await self.laixi.swipe(
                    device_id, 0.5, 0.7, 0.5, 0.3, duration_ms=random.randint(300, 500)
                )
    ...
    async def _handle_ads(self, device_id: str, timeout: int = 30) -> None:
        ...
        try:
            ...
            while (asyncio.get_event_loop().time() - start_time) < timeout:
                # 광고 스킵 버튼 클릭 시도
                await self.laixi.tap(device_id, self.coords.ad_skip[0], self.coords.ad_skip[1])
                await asyncio.sleep(0.5)

                # 대체 위치도 시도
                await self.laixi.tap(
                    device_id, self.coords.ad_skip_alt[0], self.coords.ad_skip_alt[1]
                )
    ...
    async def _watch_video(self, device_id: str, target_seconds: int, video_duration: int) -> int:
        """
        영상 시청
        ...
        try:
            ...
            while watched < target_seconds:
                # 세그먼트 시청
                segment = min(random.randint(segment_min, segment_max), target_seconds - watched)
                ...
                if action < 0.1:
                    ...
                    await self.laixi.tap(
                        device_id, self.coords.player_center[0], self.coords.player_center[1]
                    )
                    ...
                    # 10% 확률로 살짝 스크롤
                    await self.laixi.swipe(device_id, 0.5, 0.5, 0.5, 0.45, duration_ms=200)
    ...
    async def _click_like(self, device_id: str) -> bool:
        ...
        try:
            # 영상 정보 영역으로 스크롤
            await self.laixi.swipe(device_id, 0.5, 0.5, 0.5, 0.35, duration_ms=300)
            await asyncio.sleep(0.5)

            # 좋아요 버튼 클릭
            await self.laixi.tap(device_id, self.coords.like_button[0], self.coords.like_button[1])
            await asyncio.sleep(0.3)
    ...
    async def _write_comment(self, device_id: str, comment_text: str) -> bool:
        """댓글 작성"""
        try:
            # 댓글 섹션으로 스크롤
            await self.laixi.swipe(device_id, 0.5, 0.7, 0.5, 0.3, duration_ms=400)
            await asyncio.sleep(1.0)

            # 댓글 입력창 탭
            await self.laixi.tap(
                device_id, self.coords.comment_section[0], self.coords.comment_section[1]
            )
            ...
            # 댓글 입력창 탭
            await self.laixi.tap(
                device_id, self.coords.comment_input[0], self.coords.comment_input[1]
            )
            await asyncio.sleep(0.5)

            # 붙여넣기
            await self.laixi.execute_adb(device_id, "input keyevent 279")
            await asyncio.sleep(random.uniform(0.5, 1.0))

            # 제출 버튼 클릭
            await self.laixi.tap(
                device_id, self.coords.comment_submit[0], self.coords.comment_submit[1]
            )
            await asyncio.sleep(1.0)
...
# 배치 실행용 함수
async def execute_batch(tasks: List[WatchTask], concurrency: int = 5) -> List[WatchResult]:
    """
    여러 작업 배치 실행
