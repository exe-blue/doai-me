...
class DeviceRegistry:
    ...
    async def update_workstation_status(
        ...
        """
        try:
            update_data: Dict[str, Any] = {"last_heartbeat": datetime.now(timezone.utc).isoformat()}
            ...
            result = (
                self.client.table("workstations")
                .update(update_data)
    ...
    async def get_phoneboards(
        self, workstation_id: Optional[str] = None, status: Optional[str] = None
    ) -> List[PhoneboardInfo]:
        """
        폰보드 목록 조회
    ...
    async def update_phoneboard_status(
        self, phoneboard_id: str, status: str, connected_count: Optional[int] = None
    ) -> bool:
        """폰보드 상태 업데이트"""
        try:
    ...
    async def register_device(
        self, serial: str, workstation: str, board: int, slot: int, model: Optional[str] = None
    ) -> Optional[DeviceInfo]:
        """
        디바이스 등록 (폰보드-슬롯 기반)
        ...
        try:
            ...
            result = (
                self.client.table("devices").upsert(data, on_conflict="serial_number").execute()
            )
    ...
    async def bulk_register_devices(self, devices: List[Dict[str, Any]]) -> List[DeviceInfo]:
        """
        여러 디바이스 일괄 등록
    ...
    async def get_available_devices(
        ...
        return await self.get_devices(
            workstation_id=workstation_id, status="idle", group=group, limit=count
        )
    ...
    async def get_batch_groups(
        ...
        """
        group_a = await self.get_available_devices(workstation_id=workstation_id, group="A")
        group_b = await self.get_available_devices(workstation_id=workstation_id, group="B")
    ...
    async def set_device_status(
        ...
        try:
            ...
            if "-PB" in device_id and "-S" in device_id:
                # hierarchy_id
                query = (
                    self.client.table("devices").update(update_data).eq("hierarchy_id", device_id)
                )
                ...
                # serial_number
                query = (
                    self.client.table("devices").update(update_data).eq("serial_number", device_id)
                )
    ...
    async def mark_offline_stale_devices(self, stale_threshold_seconds: int = 300) -> int:
        """
        ...
        try:
            ...
            result = (
                ...
                .lt("last_heartbeat", threshold.isoformat())
                .neq("status", "offline")
                .execute()
            )
    ...
    async def update_device_command(
        self, device_id: str, command: str, result: Optional[str] = None
    ) -> bool:
        """
        디바이스 마지막 명령 정보 업데이트
        ...
        try:
            ...
            self.client.table("devices").update(update_data).eq(
                "serial_number" if "-" not in device_id else "hierarchy_id", device_id
            ).execute()
    ...
    async def get_device_stats(self, workstation_id: Optional[str] = None) -> Dict[str, int]:
        """
        디바이스 상태별 통계
        ...
        try:
            ...
            stats = {"total": len(devices), "idle": 0, "busy": 0, "offline": 0, "error": 0}
