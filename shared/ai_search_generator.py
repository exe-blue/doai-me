...
class AISearchGenerator:
    ...
    async def generate_batch(
        self, count: int = 5, categories: Optional[List[str]] = None
        ...
        for i in range(count):
            ...
            try:
                keyword = await self.generate(category=category, exclude_keywords=used_keywords)
    ...
    async def get_recent_keywords(self, limit: int = 20) -> List[str]:
        ...
        try:
            result = (
                self.client.table("ai_search_logs")
    ...
    @staticmethod
    def _build_prompt(
        category: Optional[str], context: Optional[str], exclude_keywords: Optional[List[str]]
    ) -> str:
        """프롬프트 구성"""
        ...
        if category and category in CATEGORY_PROMPTS:
            base_prompt = base_prompt.replace("검색어:", f"{CATEGORY_PROMPTS[category]}\n\n검색어:")

        # 컨텍스트 추가
        if context:
            base_prompt = base_prompt.replace("검색어:", f"추가 조건: {context}\n\n검색어:")

        # 제외 키워드 추가
        if exclude_keywords:
            exclude_str = ", ".join(exclude_keywords[:10])
            base_prompt = base_prompt.replace(
                "검색어:", f"다음 키워드는 피해주세요: {exclude_str}\n\n검색어:"
            )
    ...
    async def _generate_with_openai(self, prompt: str) -> Optional[str]:
        ...
        response = await self._openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 유튜브 검색어를 제안하는 도우미입니다. 짧고 자연스러운 한국어 검색어만 출력하세요.",
                },
    ...
    @staticmethod
    def _get_fallback_keyword(exclude_keywords: Optional[List[str]] = None) -> str:
        """폴백 검색어 가져오기"""
    ...
    async def mark_keyword_used(self, keyword: str) -> None:
        ...
        try:
            self.client.table("ai_search_logs").update(
                {"was_used": True, "used_at": datetime.now(timezone.utc).isoformat()}
