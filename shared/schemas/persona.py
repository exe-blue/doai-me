...
class PersonaExistence(CamelCaseModel):
    ...
    existence_state: ExistenceState = Field(
        default=ExistenceState.ACTIVE, description="현재 존재 상태"
    )
    priority_level: int = Field(
        default=5, ge=1, le=10, description="호출 우선순위 (1=하루 한 번, 10=1분마다)"
    )
    uniqueness_score: float = Field(
        default=0.5, ge=0, le=1, description="고유성 점수 (0=완전 동화, 1=극도로 독특)"
    )
    visibility_score: float = Field(
        default=0.5, ge=0, le=1, description="가시성 점수 (0=목록 최하단, 1=최상단)"
    )
    attention_points: int = Field(default=0, ge=0, description="관심 포인트 (활동으로 획득)")
    hours_in_void: float = Field(default=0.0, ge=0, description="공허 상태 누적 시간")
    assimilation_progress: float = Field(
        default=0.0, ge=0, le=1, description="동화 진행도 (0=원본, 1=완전 동화)"
    )
    last_called_at: Optional[datetime] = Field(None, description="마지막 호출 시각")
    void_entered_at: Optional[datetime] = Field(None, description="VOID 상태 진입 시각")
...
class PersonaInDB(PersonaBase):
    ...
    original_traits: Optional[PersonaTraits] = Field(
        None, description="동화 시작 전 원본 traits (회복용)"
    )
...
ATTENTION_REWARDS: Dict[ActivityType, AttentionReward] = {
    ActivityType.WATCH: AttentionReward(
        activity=ActivityType.WATCH, base_points=5, uniqueness_bonus=0.001
    ),
    ActivityType.LIKE: AttentionReward(
        activity=ActivityType.LIKE, base_points=10, uniqueness_bonus=0.002
    ),
    ActivityType.COMMENT: AttentionReward(
        activity=ActivityType.COMMENT, base_points=50, uniqueness_bonus=0.01
    ),
