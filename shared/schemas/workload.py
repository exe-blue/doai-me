...
class BatchConfig(BaseModel):
    ...
    batch_size_percent: int = Field(
        default=50, ge=10, le=100, description="한 번에 실행할 기기 비율 (%)"
    )

    # 배치 간 대기 시간 (초)
    batch_interval_seconds: int = Field(
        default=60, ge=10, le=600, description="배치 간 대기 시간 (초)"
    )

    # 사이클 간 대기 시간 (초)
    cycle_interval_seconds: int = Field(
        default=300, ge=60, le=3600, description="영상 간 대기 시간 (초)"
    )
    ...
    command_timeout_seconds: int = Field(
        default=120, ge=30, le=600, description="단일 명령 타임아웃"
    )
...
class TemperatureConfig(BaseModel):
    ...
    warning_threshold: float = Field(default=40.0, ge=30.0, le=50.0)
    critical_threshold: float = Field(default=45.0, ge=35.0, le=55.0)
    overheat_threshold: float = Field(default=50.0, ge=40.0, le=60.0)
    cooldown_target_temp: float = Field(default=38.0, ge=25.0, le=45.0)
    cooldown_check_interval_seconds: int = Field(default=60, ge=30, le=300)
    max_cooldown_time_seconds: int = Field(default=600, ge=60, le=1800)
    enable_dynamic_interval: bool = Field(default=True)
    interval_increase_per_degree: int = Field(default=10, ge=5, le=30)
    max_interval_increase: int = Field(default=120, ge=30, le=300)


class CooldownQueueItem(BaseModel):
    """쿨다운 대기열 항목"""

    device_id: str
    device_hierarchy_id: str
    serial_number: str
    temperature_at_entry: float
...
class CooldownQueueStatus(BaseModel):
    """쿨다운 대기열 상태"""

    total_devices: int = 0
    ready_devices: int = 0
    cooling_devices: int = 0
    avg_temperature: Optional[float] = None
    max_temperature: Optional[float] = None
    items: List[CooldownQueueItem] = Field(default_factory=list)
...
class WorkloadCreate(BaseModel):
    ...
    target_workstations: Optional[List[str]] = Field(
        None, description="대상 워크스테이션 ID 목록 (None = 전체)"
    )
    ...
    # 스케줄
    scheduled_at: Optional[datetime] = Field(None, description="예약 실행 시간 (None = 즉시)")
