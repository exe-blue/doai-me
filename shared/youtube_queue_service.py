...
class YouTubeQueueService:
    ...
    async def add_videos_from_channel(
        self, channel_id: str, videos: List[Dict[str, Any]], **kwargs
    ...
    async def get_queue_item(self, item_id: str) -> Optional[VideoQueueResponse]:
        ...
        try:
            result = (
                self.client.table("video_queue").select("*").eq("id", item_id).single().execute()
            )
    ...
    async def update_queue_item(
        self, item_id: str, update: VideoQueueUpdate
        ...
        try:
            ...
            result = (
                self.client.table("video_queue").update(update_data).eq("id", item_id).execute()
            )
    ...
    async def get_next_ready_item(self) -> Optional[VideoQueueResponse]:
        ...
        try:
            ...
            self.client.table("video_queue").update({"status": QueueStatus.READY.value}).eq(
            ...
            result = (
                self.client.table("video_queue")
                .select("*")
                .eq("status", QueueStatus.READY.value)
            ...
            if result.data:
                ...
                if item["completed_executions"] >= item["target_executions"]:
                    ...
                    self.client.table("video_queue").update(
                        {"status": QueueStatus.COMPLETED.value, "completed_at": now}
    ...
    async def record_execution(self, log: ExecutionLogCreate) -> ExecutionLogInDB:
        """
        실행 결과 기록
        ...
        return ExecutionLogInDB(
            **log.model_dump(), id=log_id, watch_percent=watch_percent, completed_at=now
        )
    ...
    async def get_random_comment(
        self, category: Optional[str] = None, language: str = "ko"
        ...
        try:
            query = (
                self.client.table("comment_pool")
    ...
    async def add_comment(self, comment: CommentPoolCreate) -> CommentPoolInDB:
        ...
        return CommentPoolInDB(
            **comment.model_dump(), id=comment_id, use_count=0, is_active=True, created_at=now
        )
    ...
    async def get_queue_summary(self) -> QueueSummary:
        """대기열 상태 요약"""
        try:
            self.client.table("video_queue").select("status", count="exact").execute()
    ...
    async def get_daily_stats(self, days: int = 7) -> List[DailyExecutionStats]:
        """일별 실행 통계"""
        try:
            result = self.client.table("daily_execution_stats").select("*").limit(days).execute()

            return [DailyExecutionStats(**stat) for stat in (result.data or [])]
        except Exception as e:
    ...
    @staticmethod
    def calculate_like_probability(base_probability: float, view_count: Optional[int]) -> float:
        """
        좋아요 확률 계산 (조회수 기반 조정)
    ...
    @staticmethod
    def calculate_comment_probability(base_probability: float, view_count: Optional[int]) -> float:
        """
        댓글 확률 계산 (조회수 기반 조정)
    ...
    @staticmethod
    def should_like(probability: float, is_logged_in: bool) -> bool:
        """
        좋아요 실행 여부 결정
    ...
    @staticmethod
    def should_comment(probability: float, is_logged_in: bool) -> bool:
        """
        댓글 실행 여부 결정
    ...
    @staticmethod
    def calculate_watch_percent(watch_duration_seconds: int, target_duration_seconds: int) -> float:
        """
        시청률 계산
    ...
    def _to_response(self, data: Dict[str, Any]) -> VideoQueueResponse:
        ...
        return VideoQueueResponse(
            ...
            first_executed_at=(
                datetime.fromisoformat(data["first_executed_at"])
                if data.get("first_executed_at")
                else None
            ),
            completed_at=(
    ...
    def _log_to_schema(self, data: Dict[str, Any]) -> ExecutionLogInDB:
        ...
        return ExecutionLogInDB(
            ...
            search_keyword=data.get("search_keyword"),
            search_method=(
                SearchMethod(data["search_method"]) if data.get("search_method") else None
            ),
