...
async function loadTasks() {
    try {
        ...
        showToast('태스크 로드 실패: ' + error.message, 'error');
    }
}
...
function openNewTaskModal() {
    ...
    if (form) {
        ...
        // Set defaults
        document.getElementById('like-probability').value = 30;
        document.getElementById('like-value').textContent = '30%';
        document.getElementById('comment-probability').value = 10;
        document.getElementById('comment-value').textContent = '10%';
        document.getElementById('subscribe-probability').value = 5;
        document.getElementById('subscribe-value').textContent = '5%';
    }
    
    document.getElementById('modal-title').textContent = '새 YouTube 태스크';
...
async function saveTask() {
    ...
    const data = {
        ...
        agent_start: parseInt(document.getElementById('agent-start').value),
        agent_end: parseInt(document.getElementById('agent-end').value),
        like_probability: parseInt(document.getElementById('like-probability').value) / 100,
        comment_probability: parseInt(document.getElementById('comment-probability').value) / 100,
        subscribe_probability: parseInt(document.getElementById('subscribe-probability').value) / 100,
        watch_duration_min: parseInt(document.getElementById('watch-min').value),
        watch_duration_max: parseInt(document.getElementById('watch-max').value),
        use_ai_comment: document.getElementById('use-ai-comment').checked,
        memo: document.getElementById('memo').value || null
    };
    
    // Validation
    if (!data.keyword || !data.title || !data.scheduled_time) {
        showToast('필수 항목을 입력해주세요', 'warning');
        return;
    }
    
    if (data.agent_start > data.agent_end) {
        showToast('에이전트 범위가 올바르지 않습니다', 'warning');
        return;
    }
...
function openNewTaskModal(addToBatch = false) {
    ...
    // 모달 제목 변경
    if (addToBatch) {
        modalTitle.textContent = '배치에 태스크 추가';
    } else {
        modalTitle.textContent = '새 YouTube 태스크';
    }
    ...
    document.getElementById('comment-value').textContent = '10%';
    document.getElementById('subscribe-value').textContent = '5%';
    document.getElementById('agent-start').value = 1;
    document.getElementById('agent-end').value = 600;
    document.getElementById('watch-min').value = 30;
    document.getElementById('watch-max').value = 180;
    document.getElementById('use-ai-comment').checked = true;
    document.getElementById('scheduled-time').value = '즉시';
...
function getFormData() {
    return {
        ...
        ai_comment_enabled: document.getElementById('use-ai-comment').checked,
        comment_text: null,
        memo: document.getElementById('memo').value || null
    };
}
